...
- **Менеджер завантажень** (Ctrl+J або меню ⋮ → Завантаження): файли тихо падають у папку Downloads, список — вбудована сторінка.
- **Копіювати адресу** (Ctrl+Shift+C), кнопка **+** нової вкладки в таббарі, Ctrl+H — історія.
...
- **Режим читання** (Ctrl+Shift+R) — чистий glass-переглядач статті, A−/A+.
- **Скріншот PNG** і **Зберегти як PDF**, **переклад сторінки** (→ укр).
- **Приватні вкладки** (Ctrl+Shift+N), **панелі закладок/історії** з пошуком.
- Примусовий темний режим сторінок, масштаб Ctrl+/−/0, зупинка завантаження Esc.

### 🏆 Досягнення
- **26 значків-досягнень** (меню ⋮ → Досягнення або лічильник 🏆 на старті): перший запуск, 10/25 вкладок, 50/250/1000 блоків, Konami, Слава Україні, закладки, інкогніто, читання, скріншот, PDF, переклад, Vault, 42, налаштування, Про, зум, середня кнопка, підказки, **пошук на сторінці, повний екран, калькулятор**.
- Тост «Нове досягнення!» при відкритті; прогрес зберігається в `achievements.json`.

### 🥚 Пасхалки
- **Konami-код** `↑↑↓↓←→←→BA` — золотий дощ частинок.
- Секретні команди в адресному рядку: `42`, `44`, `69`, `true`, `false`, `help`, `що`… і **«слава україні» / «glory to ukraine» / «russian warship»** — синьо-жовтий конфеті-дощ 💙💛.
